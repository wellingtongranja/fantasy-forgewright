  ...
  renderSignInButton() {
    ...
    if (availableProviders.length === 1) {
      ...
      this.element.innerHTML = `
        <button class="auth-signin-btn single-provider" type="button" title="Sign in...">
          ${this.getProviderIcon(provider.name)}
          <span class="auth-signin-text">Sign in...</span>
        </button>
      `
    } else if (availableProviders.length > 1) {
      // Multiple providers - show generic sign in button
      this.element.innerHTML = `
        <button class="auth-signin-btn multi-provider" type="button" title="Sign in...">
